...
export default function NavlogWaypointRow({ fix, index, isExpanded, onToggleExpand }: { fix: any, index: number, isExpanded: boolean, onToggleExpand: () => void }) {
  const { flightData, updateFlightData } = useFlightData();

  if (!flightData) return null;

  const navInputs = flightData?.navlog_inputs || {};
  const wpData = navInputs[index] || {};

  const handleWpInputChange = (field: string, val: string) => {
    const updatedInputs = { ...navInputs, [index]: { ...wpData, [field]: val } };
      ...
      {!isExpanded ? (
        ...
        >
          ...
          <div className="flex-[1.0] flex flex-col items-end gap-1 pr-2">
            ...
            <input 
              ...
              onBlur={(e) => handleWpInputChange("afob", e.target.value)}
          ...
          <div className="flex-[1.0] flex flex-col items-end gap-1">
            ...
            <input 
              ...
              onClick={(e) => e.stopPropagation()}
              onBlur={(e) => handleWpInputChange("ata", e.target.value)}
        ...
        <div className="bg-[#2b2b2b] border-y border-[#444] p-4 my-1 rounded-md shadow-inner animate-fade-in flex gap-4">
          ...
          <div className="flex-[2.0]">
            ...
            <div className="flex gap-2">
              <div className="flex-1">
                ...
                <input type="text" defaultValue={wpData.ex_tot || ""} onBlur={(e) => handleWpInputChange("ex_tot", e.target.value)} className="w-full p-2 bg-white text-black font-black text-right rounded outline-none focus:ring-2 focus:ring-[#00E676]" />
              </div>
              <div className="flex-1">
                <div className="text-text-muted text-[0.65rem]">CALC</div>
                <input type="text" defaultValue={wpData.ex_calc || ""} onBlur={(e) => handleWpInputChange("ex_calc", e.target.value)} className="w-full p-2 bg-white text-black font-black text-right rounded outline-none focus:ring-2 focus:ring-[#00E676]" />
              </div>
            </div>
          </div>
          ...
          <div className="flex-[1.8]">
            ...
            <textarea defaultValue={wpData.ex_notes || ""} onBlur={(e) => handleWpInputChange("ex_notes", e.target.value)} className="w-full h-[60px] p-2 bg-white text-black rounded outline-none resize-none focus:ring-2 focus:ring-[#00E676]" placeholder="Add notes"></textarea>
          </div>
          ...
          <div className="flex-[2.2]">
            ...
            <div className="flex items-center gap-2">
              ...
              <input type="text" defaultValue={wpData.ata || ""} onBlur={(e) => handleWpInputChange("ata", e.target.value)} className="w-[80px] p-2 bg-white text-black font-black text-right rounded outline-none focus:ring-2 focus:ring-[#00E676]" placeholder="z" />
            </div>
          </div>

          <div className="flex-[0.8] flex items-end">
            <button onClick={onToggleExpand} className="w-full py-3 border border-[#8fa0a6] text-white rounded font-bold hover:bg-white hover:text-black transition-colors">
              Done
            </button>
          </div>
        </div>
