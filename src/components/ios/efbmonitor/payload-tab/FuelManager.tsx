  ...
  return (
    <div className="bg-lido-800 p-6 rounded-xl border border-[#333333] shadow-lg">
      ...
      <div className="grid grid-cols-3 gap-4 bg-[#1a1a1a] border border-[#404040] p-4 rounded-lg mb-4">
        ...
        <div className="text-right"><div className="text-[0.55rem] text-[#8fa0a6] font-bold uppercase tracking-widest">FOB</div><div className="text-sm font-mono font-black text-[#2979FF]">{(fobKg / 1000).toFixed(1)} T</div></div>
      </div>
      {/*... 其餘 Input Fields 照舊 ...*/}
      <div className="space-y-4">
        <div className="bg-[#111] p-4 rounded-lg border border-[#222]">
          ...
          <input type="number" value={payload.fuel.uplift} onChange={(e) => setPayload({ ...payload, fuel: { ...payload.fuel, uplift: parseInt(e.target.value) || 0 }})} className="w-full bg-[#1a1a1a] border border-[#404040] focus:border-[#2979FF] rounded p-3 text-white outline-none font-mono text-sm" />
        </div>
        <div className="bg-[#111] p-4 rounded-lg border border-[#222]">
