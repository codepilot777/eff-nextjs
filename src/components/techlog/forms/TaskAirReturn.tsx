...
export function TaskAirReturn({ tlData, defects, updateTechLogData, setActiveTask }: any) {
  ...
  return (
    <div className="flex flex-col h-full relative">
      ...
      {isValid && (
        <button onClick={() => finalizeSector("Air Return", tlData?.tl_prep_dep || "HKG", ncArrivalFuel, { cmdr: ncCmdr, blocksOff: ncBlocksOff, takeOff: ncTakeOff, landing: ncLanding, blocksOn: ncBlocksOn, edto: ncEdto, autoland: ncAutoland, landingsCount: ncLandingsCount, overshoots: ncOvershoots, touchGo: ncTouchGo }, tlData, updateTechLogData, setActiveTask)} className="w-full mt-4 py-4 bg-[#FF1744] text-white font-black rounded-lg hover:bg-[#D50000] shadow-[0_0_15px_rgba(255,23,68,0.3)] animate-fade-in tracking-widest shrink-0">
          CONFIRM AIR RETURN
        </button>
      )}
    </div>
