...
export default function Navlog({ flightData, updateFlightData }: { flightData: any, updateFlightData: any }) {
  ...
  const navlog = flightData?.navlog || [];
  ...
  // 產生穩定且具真實感的假數據 (利用 waypoint 名稱產生 Hash)
  ...
  return (
    <div className="h-full w-full flex flex-col bg-[#0a0a0a] overflow-hidden rounded-lg border border-[#333333]">
      ...
      {/* 2. FIR 提示 */}
      <div className="bg-[#d1bfae] text-[#111] font-black font-mono text-[0.75rem] px-3 py-0.5 flex justify-between shrink-0">
        <span>FIR &nbsp;&nbsp;&nbsp; VHHK-HONG_KONG_FIR</span>
        <span>VHHK-HONG_KONG_FIR &nbsp;&nbsp;&nbsp; FIR</span>
      </div>
      ...
      <div className="flex-1 overflow-y-auto pb-10">
            ...
            <div className="flex flex-col gap-1 w-[60px]">
              {['doors', 'ready', 'out', 'off'].map((field) => (
                <input 
                  ...
                  className="w-full bg-[#007979] text-white font-mono font-black text-right border-none rounded-sm px-1 py-0.5 outline-none placeholder:text-white/50"
                />
              ))}
            </div>
          </div>
        ...
        {navlog.length === 0 ? (
          ...
          navlog.map((fix: any, i: number) => {
            ...
            const efob = fix.efob;
            const timeAccum = fix.time_accum;
            
            const mock = getDeterministicMockData(ident);
            const flStr = `FL${flightData?.cruise_alt || '350'}`;
            const zfw = flightData?.weight_zfw_ofp || 200;
            const wt = `${(zfw + efob).toFixed(1)}T`;
            
            const etaTime = formatEta(flightData?.std_z, timeAccum);
            const eetStr = `Δ+${Math.floor(timeAccum/60).toString().padStart(2,'0')}${(timeAccum%60).toString().padStart(2,'0')}`;
