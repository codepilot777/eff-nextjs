...
function WorkspaceContent() {
  ...
  // 🌟 mobileLabel：底部 6 個掣喺窄屏幕度冇位擺成隻 "COMMUNICATION"/"DASHBOARD"，
  // 用返縮寫先唔會逼到隔籬個掣個字疊埋一齊
  const navButtons = [
    { id: "DASH", label: "DASHBOARD", mobileLabel: "DASH" },
    { id: "NAVLOG", label: "NAVLOG", mobileLabel: "NAVLOG" },
    { id: "WEATHER", label: "WEATHER", mobileLabel: "WX" },
    { id: "NOTAM", label: "NOTAM", mobileLabel: "NOTAM" },
    { id: "COMMS", label: "COMMUNICATION", mobileLabel: "COMMS" },
    { id: "TECHLOG", label: "TECH LOG", mobileLabel: "TECH" },
  ];
    ...
    <div className="flex flex-col h-screen overflow-hidden bg-[#0a0a0a] text-[#e2e8f0] font-sans">
      ...
      <header className="flex-none flex items-center justify-between h-[54px] bg-[#1E1E1E] px-2 md:px-4 shadow-md z-50 border-b border-[#333] overflow-x-auto scrollbar-thin scrollbar-thumb-[#444] scrollbar-track-transparent">
        ...
        <div className="flex items-center gap-2 md:gap-4 shrink-0">
          <CathayLogo className="w-7 h-7 md:w-8 md:h-8 shrink-0" />
          ...
          <div className={`flex items-center rounded-full overflow-hidden text-[0.65rem] font-bold h-6 ml-1 md:ml-2 transition-colors shrink-0 ${isActivated ? 'bg-[#C6FF00] text-black' : 'bg-[#333] text-[#8fa0a6]'}`}>
            <div className={`px-2 md:px-3 flex items-center h-full border-r ${isActivated ? 'border-black/20' : 'border-[#444]'}`}>
              V{versionNum}
            </div>
            <div className="px-2 md:px-3 flex items-center h-full tracking-widest uppercase hidden sm:flex">
              {statusText}
            </div>
          </div>
        </div>
        ...
        {/* 中間：Route & Times */}
        <div className="flex items-center gap-3 md:gap-6 text-sm font-bold ml-3 md:ml-6 flex-1 shrink-0">
          {/* 🌟 日期/星期：次要資訊，mobile 收埋 */}
          <div className="hidden md:flex flex-col items-center leading-none pr-4 border-r border-[#444]">
          ...
          <div className="flex items-center gap-2 md:gap-3 shrink-0">
            <div className="flex flex-col items-start leading-none gap-0.5">
              <div className="flex items-baseline gap-1.5 md:gap-2">
                <span className="text-white text-[0.85rem] md:text-[0.95rem]">{depIcao}</span>
                <span className="text-white text-[0.7rem] md:text-xs">{stdZ}</span>
              </div>
              <div className="flex items-baseline gap-1.5 md:gap-2">
                <span className="text-[#8fa0a6] text-[0.65rem] md:text-[0.7rem]">{depIata}</span>
                <span className="text-[#8fa0a6] text-[0.65rem] md:text-[0.7rem] hidden sm:inline">{stdL}</span>
              </div>
            </div>

            <span className="text-[#8fa0a6] text-base md:text-lg mx-0.5 md:mx-1">✈️</span>

            <div className="flex flex-col items-start leading-none gap-0.5">
              <div className="flex items-baseline gap-1.5 md:gap-2">
                <span className="text-white text-[0.85rem] md:text-[0.95rem]">{arrIcao}</span>
                <span className="text-white text-[0.7rem] md:text-xs">{staZ}</span>
              </div>
              <div className="flex items-baseline gap-1.5 md:gap-2">
                <span className="text-[#8fa0a6] text-[0.65rem] md:text-[0.7rem]">{arrIata}</span>
                <span className="text-[#8fa0a6] text-[0.65rem] md:text-[0.7rem] hidden sm:inline">{staL}</span>
              </div>
            </div>
          </div>

          {/* 🌟 Flight/Block time：次要資訊，mobile 收埋 */}
        ...
        <div className="flex items-center gap-2 md:gap-4 shrink-0">
          <input
            type="text"
            placeholder="Notepad"
            className="bg-[#2A2A2A] text-white px-3 py-1.5 rounded-md text-[0.8rem] border-none outline-none w-48 placeholder:text-[#555] hidden md:block"
          />

          <div className="flex items-center gap-2 md:gap-3">
            <div className={`flex items-center justify-center w-7 h-7 md:w-8 md:h-8 rounded-full ${isSyncing ? 'text-[#00E676]' : 'text-[#8fa0a6]'}`}>
              <svg fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="w-5 h-5">
                <path strokeLinecap="round" strokeLinejoin="round" d="M12 16.5V9.75m0 0l3 3m-3-3l-3 3M6.75 19.5a4.5 4.5 0 01-1.41-8.775 5.25 5.25 0 0110.233-2.33 3 3 0 013.758 3.848A3.752 3.752 0 0118 19.5H6.75z" />
              </svg>
            </div>
            <div className="flex items-center justify-center w-6 h-6 rounded-full bg-[#C6FF00] text-black shrink-0">
              <svg fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="w-4 h-4">
                <path strokeLinecap="round" strokeLinejoin="round" d="M15.75 6a3.75 3.75 0 11-7.5 0 3.75 3.75 0 017.5 0zM4.501 20.118a7.5 7.5 0 0114.998 0A17.933 17.933 0 0112 21.75c-2.676 0-5.216-.584-7.499-1.632z" />
              </svg>
            </div>
            <div className="flex items-center justify-center w-7 h-7 md:w-8 md:h-8 rounded-full text-white bg-[#333] shrink-0">
              <svg fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="w-4 h-4">
      ...
      {/* 🌟 主內容區 */}
      <main className="flex-1 flex flex-col overflow-hidden min-h-0 relative bg-[#0a0a0a] pt-2 px-2 pb-2 md:pt-4 md:px-4">
      ...
      {/* 🌟 底部 Tabs — mobile 6 個掣塞唔晒一屏闊度，收窄 padding/字體，
          再加 overflow-x-auto 做保底（就算極窄屏幕都唔會整個掣俾切埋） */}
      <footer className="flex-none flex items-center justify-around gap-1 bg-[#121212] border-t border-[#333] px-1 md:px-2 py-1.5 pb-[max(8px,env(safe-area-inset-bottom))] z-50 overflow-x-auto scrollbar-thin scrollbar-thumb-[#444] scrollbar-track-transparent">
        {navButtons.map((btn) => {
          ...
          return (
            <button
              ...
              className={`
                flex-1 min-w-0 h-[42px] flex items-center justify-center rounded transition-all duration-200 shrink-0 overflow-hidden px-0.5
                ${isActive ? "bg-[#333333] text-white" : "bg-transparent text-[#8fa0a6] hover:bg-[#1E1E1E] hover:text-white"}
              `}
            >
              <span className="text-[0.6rem] md:text-xs font-bold tracking-wider uppercase text-center leading-tight whitespace-nowrap overflow-hidden text-ellipsis">
