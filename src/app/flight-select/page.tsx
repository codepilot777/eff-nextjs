...
function FlightSelectContent() {
  ...
  return (
    <div className="min-h-screen bg-lido-950 text-slate-300 font-sans p-8 flex flex-col">
      <h1 className="text-3xl font-black text-status-teal mb-2">
        {role === "Instructor" ? "📡 IOS - Active Sessions" : "👨‍✈️ EFB - Select Flight"}
      </h1>
      <h2 className="text-text-muted font-bold mb-6">Cathay Pacific EFF Dispatch System</h2>

      {isLoading ? (
        <div className="text-status-teal animate-pulse font-bold text-xl flex-1 flex items-center justify-center">Loading flights from Database...</div>
      ) : (
        <div className="flex flex-1 gap-6 min-h-[60vh]">
          ...
          {/* 左欄：航班清單 (Column 1)                  */}
          {/* ========================================== */}
          <div className="flex-[1] bg-lido-800 border border-[#333333] rounded-xl p-4 flex flex-col shadow-lg">
            <h4 className="text-white font-bold text-sm uppercase mb-4 tracking-wider">🛫 Available Flights</h4>
            {flights.length === 0 ? (
              <div className="text-text-muted text-sm text-center mt-10">No flights found in database.</div>
            ) : (
                    ...
                    <button
                      ...
                      className={`w-full py-4 px-4 rounded-lg font-bold text-left transition-all flex justify-between items-center ${
                        isSelected 
                          ...
                          : "bg-lido-800 text-white hover:border-[#00bfa5] border border-transparent"
                      }`}
                    >
                      <span>✈️ {f.flight_no}</span>
                      <span className={`text-xs px-2 py-1 rounded ${isSelected ? 'bg-black/20' : 'bg-lido-800'}`}>
                        {f.dep_icao}
                      </span>
                    </button>
                  );
          ...
          {/* 統一右側大容器：包含版本選擇與詳細資料       */}
          {/* ========================================== */}
          <div className="flex-[3] bg-lido-800 border border-[#333333] rounded-xl flex overflow-hidden shadow-lg">
            {!selectedFlight ? (
              <div className="w-full h-full flex items-center justify-center text-text-muted italic text-lg bg-[#0a0a0a]/30">
                👈 Select a flight from the left column to view details.
              </div>
            ) : (
              <>
                {/* 內左欄：OFP 版本選擇 */}
                <div className="flex-[1] border-r border-[#333333] p-4 flex flex-col bg-[#0a0a0a]/40">
                        ...
                        <button
                          ...
                          className={`w-full p-4 rounded-lg text-left transition-all border ${
                            isVerSelected 
                              ...
                              : "bg-[#0a0a0a] border-[#333333] hover:border-[#8fa0a6]"
                          }`}
                        >
                          <div className="flex justify-between items-center mb-1">
                            <span className={`font-black text-xl ${isVerSelected ? 'text-status-teal' : 'text-white'}`}>
                              V{v}
                            </span>
                            {isActivated && (
                ...
                {/* 內右欄：SNN 與 動作按鈕 */}
                <div className="flex-[2.5] p-6 flex flex-col relative bg-lido-800">
                  <div className="flex flex-col h-full animate-fade-in">
                    ...
                    <div className="flex justify-between items-start mb-6 border-b border-[#333333] pb-4">
                      <div>
                        ...
                        <div className="text-status-teal font-bold tracking-widest mt-1 text-lg">
                          {selectedFlight.dep_icao} ➔ {selectedFlight.arr_icao}
                        </div>
                      </div>
                      <div className="text-right">
                        <div className="text-text-main font-bold text-lg">{selectedFlight.aircraft_reg}</div>
                        <div className="text-text-muted text-sm">{selectedFlight.aircraft_type}</div>
                      </div>
                    </div>

                    {/* SNN 顯示區 */}
                    <h5 className="text-white font-bold text-sm uppercase mb-2">📝 Special Navigation Notes (SNN)</h5>
                    <div className="flex-1 bg-[#0a0a0a] border border-[#404040] rounded-lg p-5 overflow-y-auto mb-6 shadow-inner font-mono text-sm text-text-main leading-relaxed whitespace-pre-wrap">
                      ...
                      <div className="flex-1">
                        {selectedFlight.activated_version === selectedVersion ? (
                           ...
                           <button 
                             ...
                             className="w-full py-4 bg-lido-800 border-2 border-[#00bfa5] text-status-teal rounded-xl font-black text-lg hover:bg-[#00bfa5] hover:text-black transition-colors"
                           >
...
export default function FlightSelectPage() {
  return (
    <Suspense fallback={<div className="p-8 text-status-teal">Loading...</div>}>
      <FlightSelectContent />
    </Suspense>
  );
}